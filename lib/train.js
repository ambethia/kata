...
const train = (config, kata, language) => {
  if (kata === undefined) {
    console.error('Kata is both required.')
    process.exit(1)
  }
  ...
  .end((err, response) => {
    ...
    if (response.body.success) {
      ...
      const pathToKata = path.join(language, slug)
      mkdirp(pathToKata, () => {
        fs.writeFileSync(path.join(pathToKata, ['solution', EXT[language]].join('.')), response.body.session.setup)
        fs.writeFileSync(path.join(pathToKata, ['fixtures', EXT[language]].join('.')), response.body.session.exampleFixture)
