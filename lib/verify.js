...
const verify = (pathToKata, language) => {
  ...
  const compose = path.resolve(__dirname, '../node_modules/codewars-runner-cli/docker-compose.yml')
  ...
  const cmdOpts = ['docker-compose', '-f', compose, 'run', language, '-c', solution, '-t', framework, '-f', fixtures]
  if (language === 'javascript') cmdOpts.push('--languageVersion', '8.1.3/Babel') // TODO: Configurable
